...
public class WordCount extends TezExampleBase {
	...
	public static class SumProcessor extends SimpleMRProcessor {
		...
		public void run() throws Exception {
			Preconditions.checkArgument(getInputs().size() == 1);
			Preconditions.checkArgument(getOutputs().size() == 1);
			System.out.println("SumProcessor run()");
	...
	private DAG createDAG(TezConfiguration tezConf, String inputPath, String outputPath, int numPartitions)
		...
		System.out.println("-------------------");
		System.out.println(edgeConf.getInputClassName());
		System.out.println(edgeConf.getOutputClassName());
		System.out.println("-------------------");
		...
		/*
		 * Edge e2 = Edge.create(tokenizerVertex, summationVertex,
		 * EdgeProperty.create(DataMovementType.SCATTER_GATHER,
		 * DataSourceType.PERSISTED, SchedulingType.SEQUENTIAL,
		 * OutputDescriptor.create("TokenProcessor"),
		 * InputDescriptor.create("SumProcessor")));
		 */
		...
		Edge e1 = Edge.create(tokenizerVertex, summationVertex, edgeConf.createDefaultEdgeProperty());
		DAG dag = DAG.create("WordCount");
		dag.addVertex(tokenizerVertex).addVertex(summationVertex).addEdge(e1);
		return dag;
	}
