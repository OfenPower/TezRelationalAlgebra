...
public class AppTest extends Configured implements Tool {
	...
	private int runJob(String[] args, TezConfiguration tezConf, TezClient tezClient) throws TezException, IOException {
		...
		/*
		 * // DataSource und DataSinks erzeugen DataSourceDescriptor dataSource1
		 * = MRInput .createConfigBuilder(new Configuration(tezConf),
		 * TextInputFormat.class, "artikel.csv").build(); DataSourceDescriptor
		 * schemeSource1 = MRInput .createConfigBuilder(new
		 * Configuration(tezConf), TextInputFormat.class,
		 * "artikel.scheme").build(); DataSourceDescriptor dataSource2 = MRInput
		 * .createConfigBuilder(new Configuration(tezConf),
		 * TextInputFormat.class, "liegen.csv").build(); DataSourceDescriptor
		 * schemeSource2 = MRInput .createConfigBuilder(new
		 * Configuration(tezConf), TextInputFormat.class,
		 * "liegen.scheme").build(); DataSinkDescriptor dataSink = MROutput
		 * .createConfigBuilder(new Configuration(tezConf),
		 * TextOutputFormat.class, "output.csv").build();
		 */
